...
func _ready():
	(experience_manager as ExperienceManager).level_up.connect(_on_level_up)
...
func _on_level_up(_current_level: int):
	...
	(upgrade_screen_instance as UpgradeScreen).set_ability_upgrades(chosen_upgrades)
	(upgrade_screen_instance as UpgradeScreen).upgrade_selected.connect(_on_upgrade_selected)
